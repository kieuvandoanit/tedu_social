...
class ProfileController{
    ...
    public getCurrentProfile = async(req: Request, res: Response, next: NextFunction)=>{
        try{ 
            const userId = req.user.id;
            const resultObj:Partial<IUser> = await this.profileService.getCurrentProfile(userId);
    ...
    public createProfile = async (req: Request, res: Response, next: NextFunction)=>{
        ...
        const userId = req.user.id;
        try{
            const createUserData: IProfile = await this.profileService.createProfile(userId, userData);
    ...
    public getByUserId = async (req: Request, res: Response, next: NextFunction) =>{
        ...
        try{
            const updateUserData: Partial<IUser> = await this.profileService.getCurrentProfile(userId);
    ...
    public createEducation = async(req: Request, res: Response, next: NextFunction)=>{
        ...
        const userId = req.user.id;
        try{
            const createUserData: IProfile = await this.profileService.addEducation(userId, data);
            res.status(200).json(createUserData);
        }catch(error){
            next(error);
        }
    ...
    public deleteEducation = async (req: Request, res: Response, next: NextFunction)=>{
        ...
        try{
            const profile = await this.profileService.deleteEducation(req.user.id, eduId);
            res.status(200).json(profile);
        }catch(error){
            next(error);
        }
    ...
    public follow = async (req: Request, res: Response, next: NextFunction)=> {
        const toUserId: string = req.params.to_id;
        try {
